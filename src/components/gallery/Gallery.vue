...
<template>
  <div class="gallery-wrapper">
    <div v-if="isGalleryViewOpen" class="gallery-view container">
      <div class="row justify-content-center">
        <!-- Main Galleries -->
        <div
          v-for="(gallery, index) in galleries"
          :key="index"
          class="responsive-box col-md-6 col-lg-3 gallery-base bg-transparent card me-3 mb-3 mt-4"
          @click="expandGallery(gallery)"
        >
          <img
            class="card-img img-thumbnail bg-black"
            :src="`${webURL()}${gallery.galleryCoverArtUrl}`"
        ...
        <div class="row justify-content-center">
          ...
          >
            <div class="image-card-item">
              <a
                :href="`${webURL()}${galleryItem.galleryItemUrl}`"
                class="lightbox"
              ...
              >
                <img
                  :src="`${webURL()}${galleryItem.galleryItemUrl}`"
    ...
    >
      <form enctype="multipart/form-data" @submit.prevent="createNewGallery">
        ...
        <div class="form-group">
          ...
          <input id="file" type="file" multiple @change="uploadFile($event)" />
        </div>
...
export default {
  ...
  setup() {
    ...
    const closeGalleryItemsView = computed(
      () => store.methods.closeGalleryItems
    );
    ...
    const viewGalleryItemForm = computed(
      () => store.methods.newGalleryItemForm
    );
    // Close gallery item form
    const closeGalleryItemForm = computed(
      () => store.methods.closeGalleryItemForm
    );
    ...
    const isNewGalleryFormOpen = computed(
      () => store.state.isNewGalleryFormOpen
    );
    const isNewGalleryItemFormOpen = computed(
      () => store.state.isNewGalleryItemFormOpen
    );
    ...
    async function getGalleryItems() {
      const getGalleryByName = localStorage.getItem("gallery")
    ...
    async function expandGallery(gallery) {
      ...
      if (gallery.galleryName !== "" || undefined) {
        // Then go to create gallery item screen
